...
- 智能切换：有操作收起为灵动岛，无操作展开横幅；**横幅展开后再闲置 N 秒自动弹出大屏（秒数可自定义，0=不自动）**；无计时事件时锁定灵动岛（纯黑）
- **智能隐藏**：检测到全屏（授课/视频/演示）时窗口彻底隐藏，退出全屏自动恢复（可关闭）
...
- 显示效果可选：**液态玻璃**（中心完全平整透出原画面、只有边缘一圈窄折射带 + 浅灰边线）· **真实模糊**（截屏毛玻璃）· 模拟玻璃 · 关闭
- 多事件轮播、事件置顶、文言文模式、文字随背景亮度自动适配并加描边、配置窗口暗色主题
...
- `SmartCounterIsland-Setup-1.6.0.exe` —— 安装版（创建开始菜单/桌面快捷方式）
- `SmartCounterIsland-1.6.0-portable.exe` —— 便携版（免安装，直接双击运行）
...
自检：`npm run test`（110 项自动化断言）/ `npm run smoke`（探针、毛玻璃、配置窗口）。
