...
- 三种状态窗口：**灵动岛**（默认小条）· **横幅**（无操作 4 秒自动展开，显示事件名+天数+时分秒）· **倒计时窗口**（大屏展示，高度为屏幕 1/4，宽度随文字自适应，突出天数）
- 智能切换：有操作收起为灵动岛，无操作展开横幅；**全屏状态锁定灵动岛 + 鼠标穿透**（不影响授课）；无计时事件时锁定灵动岛（纯黑）
...
- `SmartCounterIsland-Setup-1.2.0.exe` —— 安装版（创建开始菜单/桌面快捷方式）
- `SmartCounterIsland-1.2.0-portable.exe` —— 便携版（免安装，直接双击运行）
...
自检：`npm run test`（79 项自动化断言）/ `npm run smoke`（探针、毛玻璃、配置窗口）。
