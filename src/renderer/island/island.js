...
function render() {
  ...
  if (state === 'dock') {
    // 计时坞（对标 iOS 倒计时 Live Activity）：环形进度 + 超大等宽数字 + 极小标签 + 细线性进度条
    const d = dock || { title: '距离 节假日', num: '--', unit: '天', date: '', pct: 0, at: 0 };
    const pct = Math.max(0, Math.min(1, Number(d.pct) || 0));
    // 最后一天：改成逐秒的 hh:mm:ss（苹果倒计时就是这么走的）—— 这里用本地时钟算，不靠每秒推状态
    let num = d.num;
    let unit = d.unit;
    if (d.at) {
      const left = d.at - Date.now();
      if (left <= 86400000 && left > -1000) {
    ...
    box.innerHTML =
      ...
      ring +
      '<div class="dk-text">' +
      `<div class="dk-row"><span class="dk-num">${ESC(num)}</span>${unit ? `<span class="dk-unit">${ESC(unit)}</span>` : ''}</div>` +
...
/* 计时坞末段逐秒：目标在 24 小时内时每秒重画一次（不靠主进程推状态） */
setInterval(function () {
  if (state !== 'dock' || !dock || !dock.at) return;
