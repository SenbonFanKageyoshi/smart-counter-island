...
async function runShots() {
  ...
  for (const st of ['strip', 'expanded', 'zoom', 'corner', 'progress']) {
    island.setState(st);
    await new Promise((r) => setTimeout(r, 300));
...
function runTests() {
  ...
  (async () => {
    try {
      ...
      ok(
        `T24 全屏授课形态决策 (隐藏→${island.decideState({ ...fsBase, fullscreenState: 'strip' })}，角落→${island.decideState({ ...fsBase, fullscreenState: 'corner' })}，进度条→${island.decideState({ ...fsBase, fullscreenState: 'progress' })}，无事件角落→${island.decideState({ ...fsBase, fullscreenState: 'corner', hasCountdown: false })})`,
        island.decideState({ ...fsBase, fullscreenState: 'strip' }) === 'strip' &&
          ...
          island.decideState({ ...fsBase, fullscreenState: 'corner', hasCountdown: false }) === 'strip'
      );
      ...
      ok(
        `T29 内容绕开禁区（小岛内不再画那块黑） (state=${dom29.state} layout=${dom29.layout} 块元素=${dom29.hasNotchZoneEl} 禁区=${JSON.stringify(zr29)} 屏幕=${JSON.stringify(ns29.screen)} 期望屏幕=(${Math.round(spec29.zone.x)},${Math.round(spec29.zone.y)}) 左图标=${dom29.emojiLeft} 右数字=${dom29.daysRight})`,
        dom29.state === 'strip' &&
          dom29.bodyNotch === '1' &&
          dom29.layout === 'split' &&
