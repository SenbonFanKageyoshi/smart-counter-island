...
if (process.argv.includes('--diag-press') || process.argv.includes('--test') || process.argv.includes('--shot') || process.argv.includes('--shot-strip') || process.argv.includes('--shot-init') || process.argv.includes('--shot-collapse') || process.argv.includes('--shot-weather') || process.argv.includes('--shot-config') || process.argv.includes('--shot-lab') || process.argv.includes('--glass-lab') || process.argv.includes('--gpu') || process.argv.includes('--smoke') || process.argv.includes('--perf')) {
...
async function main() {
  ...
  else if (argv.includes('--diag-press')) runDiagPress();
...
// ---------------- 长按链路诊断（--diag-press：用真实输入事件按住 1 秒） ----------------
async function runDiagPress() {
  const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
  const report = path.join(os.tmpdir(), 'sci-press-report.txt');
  const lines = [];
  const log = (s) => {
  ...
  try {
    log(`[diag-press] start state=${island.state} mode=${settings.load().manual.mode}`);
    if (island.state !== 'strip') island.setState('strip');
    await sleep(900);
    const w = island.win;
    const probe = await w.webContents.executeJavaScript(`(() => { const p = document.getElementById('pill'); const r = p.getBoundingClientRect(); return { x: Math.round(r.left + r.width / 2), y: Math.round(r.top + r.height / 2), w: Math.round(r.width), h: Math.round(r.height), dbg: window.__pressDbg ? window.__pressDbg() : null }; })()`);
    log(`[diag-press] pill=${JSON.stringify(probe)}`);
    // 真实鼠标按下（经 Chromium 输入管线 → 真的 pointerdown）
    w.webContents.sendInputEvent({ type: 'mouseMove', x: probe.x, y: probe.y });
    await sleep(120);
    w.webContents.sendInputEvent({ type: 'mouseDown', x: probe.x, y: probe.y, button: 'left', clickCount: 1 });
    await sleep(1000); // 保持按住（长按阈值 700ms）
    const mid = await w.webContents.executeJavaScript(`(() => ({ dbg: window.__pressDbg ? window.__pressDbg() : null, pressing: document.body.classList.contains('pressing'), state: document.body.dataset.state }))()`);
    log(`[diag-press] 按住 1s 后：${JSON.stringify(mid)} island.state=${island.state} mode=${settings.load().manual.mode} payloadDockEdit=${island.getStatePayload().dockEdit} payloadDock=${JSON.stringify(island.getStatePayload().dock)}`);
    const inner = await w.webContents.executeJavaScript(`(() => ({ html: (document.getElementById('content') || {}).innerHTML ? document.getElementById('content').innerHTML.slice(0, 90) : '', hasWrap: !!document.querySelector('.dk-wrap'), hasChips: !!document.querySelector('.dk-edit') }))()`);
    log(`[diag-press] 坞内 DOM：${JSON.stringify(inner)}`);
    w.webContents.sendInputEvent({ type: 'mouseUp', x: probe.x, y: probe.y, button: 'left', clickCount: 1 });
    await sleep(1400);
  ...
  // 倒计时进行中长按 → 操作页：这里直接走状态机入口（避开通知抢状态的时序影响）
  try {
    if (island.state !== 'dock') island.setState('dock');
    ...
    const shotDir2 = app.isPackaged ? path.join(app.getPath('temp'), 'sci-shots') : path.join(__dirname, '..', '..', 'shots');
    fs.mkdirSync(shotDir2, { recursive: true });
    fs.writeFileSync(path.join(shotDir2, 'dock-menu.png'), (await island.win.webContents.capturePage()).toPNG());
    log('[diag-press] 操作页截图 → dock-menu.png');
  } catch (e) {
    log(`[diag-press] 操作页失败 ${e && e.message}`);
  }

  try {
    const shotDir = app.isPackaged ? path.join(app.getPath('temp'), 'sci-shots') : path.join(__dirname, '..', '..', 'shots');
    fs.mkdirSync(shotDir, { recursive: true });
    fs.writeFileSync(path.join(shotDir, 'dock.png'), (await island.win.webContents.capturePage()).toPNG());
