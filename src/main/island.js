...
function decideState(input) {
  ...
  // 计时坞：手动开了「计时坞」模式，或节假日倒计时到点 → 最高优先（全屏授课除外）
  ...
  // 横幅模式已去掉：闲置 → 直接放宽到大窗口（允许的话），否则维持细条；
  // 操作后 zoomCooldown 期间不自动弹大屏（避免"收起后又马上弹出"）
  // 两个计时合并成一层：闲置到 max(展开秒数, 大屏秒数) 且允许大屏 → 直接放宽到大窗口。
  // （以前是"闲置 expandIdleSec → 横幅，再闲置 zoomIdleSec → 大屏"，横幅去掉后合并）
  const zoomIdleMs = Math.max(0, zoomIdleSec) * 1000;
  if (zoomAllowed && !zoomCooldown && idleMs >= Math.max(expandIdleSec * 1000, zoomIdleMs)) {
    return 'zoom';
  }
  // 已经在大屏且开了自动放大（zoomIdleSec>0）：继续闲置就保持大屏，不要抖回细条；
  // 没开自动放大时，有操作就该收回细条（keeping 规则不能无条件生效）
...
class Island {
  ...
  tick() {
    ...
    const next = decideState({
      ...
      // 计时坞文案：推送时现算（不依赖 tick 缓存，避免推送时机不同导致载荷为空）
      dock: this.dockPayload() || this.dockText || null,
    });
