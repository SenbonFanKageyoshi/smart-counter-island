...
const PILL = {
  ...
  corner: { w: 180, h: 72 },   // 全屏授课：右上角角落卡片（贴屏幕右上角，只显示剩余时间）
...
/** 各状态的圆角半径（DIP；strip/expanded 为胶囊 = 高度一半，与渲染层 CSS 一致）。
    corner/progress 用 0：这两态贴屏幕边，Win32 区域取整窗矩形，形状交给 CSS。 */
const REGION_RADIUS = { strip: 13, expanded: 32, zoom: 42, notify: 24, corner: 0, progress: 0 };
...
function decideState(input) {
  const {
    ...
    fullscreenState = 'strip', // 全屏授课时的目标形态：'strip' | 'corner' | 'progress'
  ...
  if (occluded) {
    if (fullscreenState === 'corner') return 'corner';
  ...
  if (overPill && state !== 'zoom' && state !== 'corner' && state !== 'progress') return null;
...
class Island {
  ...
  getStatePayload(winBoundsOverride) {
    ...
    return { state: this.state, opacity: this.currentOpacity, geom, notch: this.cameraNotch(undefined, winBoundsOverride), weather: this.weatherPayload() };
  }
  ...
  cameraNotch(state, winBoundsOverride) {
    ...
    const layout = st === 'corner' ? 'none' : sensors.pickLayout(spec.layout, st, pillH, top);
    const hit =
  ...
  computeBounds(state, disp) {
    ...
    if (state === 'corner' || state === 'progress') {
      ...
      // 顶部进度条：整宽贴顶
      return { x: wa.x, y: wa.y, width: cs.w, height: cs.h };
    }
  ...
  /** 角落卡片 / 顶部进度条的尺寸（DIP）：卡片可按设置缩放，进度条整宽 */
  cornerSize(state, disp) {
    const wa = disp.workArea;
    if (state === 'progress') return { w: wa.width, h: Math.max(10, PILL.progress.h) };
    const s = PILL.corner;
    const w = Math.max(140, Math.min(360, s.w));
    return { w, h: Math.max(48, Math.min(160, s.h)) };
  }

  /** 全屏授课时的目标形态：'hide' | 'strip' | 'corner' | 'progress'
  ...
  tick() {
    ...
    //    'hide' 彻底隐藏（默认）| 'corner' 右上角卡片 | 'progress' 顶部进度条 | 'strip' 保持灵动岛小条
